...
namespace Tobo.Net
{
	...
	public class NetworkDiscovery : MonoBehaviour
	{
		...
		static DiscoveryInfo ReadDataFromUdpClient(UdpClient udpClient)
		{
			...
			//Debug.Log("Got data from client");
		...
		void UpdateServer()
		{
			...
			{
				Debug.Log("S: UDP NULL? " + (m_serverUdpCl == null));
				Debug.Log($"S: RUN: {runServer}");
			}
			*/
            ...
            //if (Time.frameCount % 30 == 0)
            //    Debug.Log("S: INFO NULL? " + (info == null));
            if (info != null)
		...
		{
			//Debug.Log("Got server discovery request");
		...
		{
			buf.Add((byte)dict.Count);
            foreach (string key in dict.Keys)
            {
				buf.AddString(key);
				buf.AddString(dict[key]);
            }
		}
		...
		public static void GetDiscoveryRequestData(ByteBuffer buf)
		{
			...
			buf.Add<ushort>(0); // Blank, for space
			WriteDict(buf, dict);
			Profiler.EndSample();
		}
		...
		public void Search()
		{
			...
			foreach (var address in GetBroadcastAdresses())
			{
                ...
                //Debug.Log("E: SEND BUF TO " + endPoint);
            }
